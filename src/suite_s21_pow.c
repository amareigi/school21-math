...
START_TEST(pow_5) {
  ...
  double ex = 0;
  long double orig_res = pow(num, ex), our_res = s21_pow(num, ex);
  ck_assert_int_eq(orig_res, our_res);
}
END_TEST
...
START_TEST(pow_6) {
  double num = 0;
  double ex = INFINITY;
  ck_assert_double_eq(pow(num, ex), s21_pow(num, ex));
}
END_TEST
...
START_TEST(pow_33) { ck_assert_float_eq_tol(s21_pow(2, 18), pow(2, 18), 1e-6); }
END_TEST

START_TEST(pow_34) {
  ck_assert_float_eq_tol(s21_pow(2.1, 11), pow(2.1, 11), 1e-6);
}
END_TEST
